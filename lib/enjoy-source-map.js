...
const { T_MATCHS, INLINE_SM_REG } = require('../config');
...
const SMGenerator = {
    ...
    getSourceMap(tpl, match, source) {
        ...
        if (SMGenerator.haveInlineMap(source)) {
            resMap = SMGenerator.getOffsetMap(source, SMGenerator.getOffset(tpl, match));
        }
        return resMap;
    },
    /**
    ...
    overWriteReplace(target, ...arg) {
        ...
        map = map.toObject();
        var begin = map.mappings.indexOf()
        var decoded = map.mappings.split(';').map(function(line) {
            return line.split(',');
        }).map(function(line) {
        ...
        for (var i = 0, lc = lines.length; i < lc; i++) {
            var columnDiff = lines[i].length - orgLines[i].length;
            if (i > columnDiff && decoded[i] && decoded[i][0]) {
                decoded[i][0][0] += columnDiff;
            }
        }
        map.mappings = decoded.map(function(line) {
            return line.map(vlq.encode);
        }).map(function(line) {
            return line.join();
        }).join(';');
        return convert.removeComments(result) + convert.fromObject(map).toComment();
    },
    /**
