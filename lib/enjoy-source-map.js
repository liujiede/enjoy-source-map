...
const SMGenerator = {
     ...
     */
    getSourceMap(tpl, offset, source) {
        var resMap = '';
        if (SMGenerator.hasInlineMap(source)) {
            offset = SMGenerator.getOffset(tpl, offset);
    ...
    overWriteTlp(tpl, data) {
        var sourcemap = '';
        var code = tpl.replace(consts.tpl_match, function(all, key, offset) {
            var source = data[key] || '';
            if (needSouceMap() && SMGenerator.hasInlineMap(source)) {
                if (sourcemap = (SMGenerator.getSourceMap(tpl, offset, source) || '')) {
