...
const SMGenerator = {
    ...
    concat(source1, source2, offset) {
        var commen = convert.fromBase64(combine.create('bundle.js').addFile({
            source: source1
        }).addFile({
            source: source2
        }, offset).base64()).toComment();
    ...
    overWriteTlp(tpl, data) {
        var sourcemap = '';
        var lines = 0;
        var code = tpl.replace(consts.tpl_match, function (all, key, index) {
            var source = data[key] || '';
            if (needSouceMap() && SMGenerator.hasInlineMap(source)) {
                var offset = SMGenerator.getOffset(tpl, index);
                offset.l += lines;
                var newmap = SMGenerator.getOffsetMap(source, offset) || '';
                if (newmap) {
                    if (sourcemap) {
                        //merge
                        sourcemap = SMGenerator.concat(sourcemap, source, offset);
                    } else {
                        sourcemap = newmap;
                    }
