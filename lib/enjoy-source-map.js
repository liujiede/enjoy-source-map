...
const SMGenerator = {
    ...
    concatSMFiles(filesCol) {
        ...
        filesCol.reduce(function (a, b) {
            var lines = convert.removeComments(b.source).split('\n');
            lines.pop();//去除空行
            codes.push(lines.join('\n'))//载入源代码
            var preLine = a.source ? a.source.split('\n').length - 1 : 0;
            b.offset.line += a.offset.line + preLine;//计算偏移量
    ...
    concatUnmapFile(arr) {
        ...
        if (pre) {
            let preHaveInlineCode = res[res.length - 1].source;
            let comment = consts.inline_sm.exec(preHaveInlineCode)[0];
            res[res.length - 1].source = convert.removeComments(preHaveInlineCode) + pre + '\n' + comment;
        }
        return res;
    },
    /**
